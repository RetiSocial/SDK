...
class RetiClient:
    ...
    def _handle_group(self, msg_data: Dict[str, Any]):
        ...
        if msg_type == "group_invite":
            ...
            self._storage.add_group_member(group_id, self.identity_hash or "", self._get_display_name())
            ...
            if source not in [m["identity_hash"] for m in members]:
                return
    ...
    def send_dm(self, destination: str, content: str, thread_id: Optional[str] = None,
        ...
        success = self._transport.send_message(destination, content, title="", fields=fields)
        ...
        if success:
            ...
            message_hash = self._storage.save_message(msg_data, is_outbound=True)
            
            if attachment_list and message_hash:
    ...
    def accept_follow(self, identity_hash: str) -> bool:
    ...
    def create_group(self, name: str, members: List[str]) -> Optional[str]:
        ...
        group_id = hashlib.sha256(f"{self.identity_hash}{name}{time.time()}".encode()).hexdigest()
        
        self._storage.create_group(group_id, name, self.identity_hash, is_admin=True)
        self._storage.add_group_member(group_id, self.identity_hash, self._get_display_name())
    ...
    def send_group_message(self, group_id: str, content: str, 
        ...
        success_count = 0
        for member in members:
            if member["identity_hash"] != self.identity_hash:
                if self._transport.send_message(member["identity_hash"], content, title="group_message", fields=fields):
                    success_count += 1
    ...
    def leave_group(self, group_id: str) -> bool:
        """Leave a group."""
        group = self._storage.get_group(group_id)
        if not group or not self.identity_hash:
            return False
        ...
        self._storage.remove_group_member(group_id, self.identity_hash)
        self._storage.delete_group(group_id)
        return True
    ...
    def kick_member(self, group_id: str, identity_hash: str) -> bool:
        ...
        members = self._storage.get_group_members(group_id)
        for member in members:
            if member["identity_hash"] not in (self.identity_hash, identity_hash):
